...
func (u *Upgrade) releasingUpgrade(c chan<- resultMessage, upgradedRelease *release.Release, current kube.ResourceList, target kube.ResourceList, originalRelease *release.Release) {
	...
	if ssa, ok := u.cfg.KubeClient.(kube.InterfaceWithSSA); ok && u.ServerSideApply {
		u.cfg.Log("Using server-side apply for %s", upgradedRelease.Name)
		results, err = ssa.Apply(current, target, u.Force)
	} else {
